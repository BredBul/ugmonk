...
if (categoriesItem.classList.contains('_active')) {
  categories.style.setProperty(
    "--underline-width",
    `calc(${categoriesItem.offsetWidth}px + 10px)`
  );
  categories.style.setProperty(
    "--underline-offset-x",
    `calc(${categoriesItem.offsetLeft}px - 5px)`
  );
}
categories.addEventListener("click", (event) => {
  if (event.target.classList.contains("categories__item")) {
    categories.style.setProperty(
      "--underline-width",
      `calc(${event.target.offsetWidth}px + 10px)`
    );
    categories.style.setProperty(
      "--underline-offset-x",
      `calc(${event.target.offsetLeft}px - 5px)`
    );
  }
});
